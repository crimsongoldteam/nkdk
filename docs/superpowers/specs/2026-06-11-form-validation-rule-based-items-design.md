...
### Уточнение Контракта `defaultValueYAML`

`defaultValueYAML` не является значением, которое нужно подставлять при `YAML -> модель`.
Его роль — сделать YAML компактным при выгрузке и строгим при validation: значение,
равное `defaultValueYAML`, не пишется в YAML, а если ключ присутствует явно, schema
...
- специальных флагов для “восстановления” `defaultValueYAML` при импорте быть не должно.
