...
Текущее правило `DynamicListRules.customQuery` задаёт `implicitValueYAML: false`. Это верно: значение `false` является YAML-default и не должно выгружаться. Поэтому validation schema ожидает, что если ключ `ПроизвольныйЗапрос` присутствует, то допустимое явное значение только `Истина`.

Проблема не в schema. Проблема в выгрузке YAML: `boolean.toYAML` превращает модельное `false` в строку `Ложь`, а общий фильтр default-значений сравнивает уже YAML-строку `"Ложь"` с модельным `implicitValueYAML: false`. Значения не равны, поэтому default не отсекается и попадает в YAML.
...
- отсекать `implicitValueYAML` до преобразования значения в YAML-представление или сравнивать default в той же форме, что и экспортированное значение;
...
### Уточнение Контракта `implicitValueYAML`

`implicitValueYAML` не является значением, которое нужно подставлять при `YAML -> модель`.
Его роль — сделать YAML компактным при выгрузке и строгим при validation: значение,
равное `implicitValueYAML`, не пишется в YAML, а если ключ присутствует явно, schema
может ограничивать допустимую явную форму.

Если ключ отсутствует в YAML, соответствующее свойство отсутствует и в модели,
если только значение не пришло из `source`/reference fallback или внешнего файла.
Общие модельные `defaultValue` также не должны использоваться для достраивания
сокращённого YAML. Если значение нужно для XML-цикла, но не должно появляться из
отсутствующего YAML-ключа, `defaultValue` должен быть ограничен по операции или
значение должно жить в XML-default правиле. XML-default должен восстанавливаться
на границе `модель -> XML` через существующие механизмы `defaultValueXML`,
`defaultValueXMLRaw` и reference-данные.

Следствие для сокращённого YAML:

- отсутствие `РежимОтображения: Обычный` не создаёт `viewMode: "Normal"` в модели;
- отсутствие `ВидСравнения: Равно` не создаёт `comparisonType: "Equal"` в модели;
- отсутствие `ПроизвольныйЗапрос: Ложь` не создаёт `customQuery: false` в модели;
- специальных флагов для “восстановления” `implicitValueYAML` при импорте быть не должно.
