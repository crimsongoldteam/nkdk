...
| - Ожидание результата подготовки | 7.80s | 7.80s | - | - | - | - | 5304 MiB | - | - | - |
|   - Выполнение подготовки в worker | 4.06s | - | 4.03s | 4.04s | 4.06s | 16.18s | - | 1556 MiB | 1562 MiB | 1582 MiB |
...
| Первичная проверка YAML | 16.61s | - | 14.74s | 15.96s | 16.61s | 63.82s | - | 3572 MiB | 3655 MiB | 3682 MiB |
| - Проверка JSON Schema | 8.50s | - | 7.90s | 8.12s | 8.50s | 32.48s | - | 3572 MiB | 3655 MiB | 3682 MiB |
| - Построение модели | 4.20s | - | 3.80s | 4.00s | 4.20s | 16.00s | - | 3572 MiB | 3655 MiB | 3682 MiB |
| - Построение member index | 1.10s | - | 0.90s | 1.00s | 1.10s | 4.00s | - | 3572 MiB | 3655 MiB | 3682 MiB |
...
`Ожидание результата подготовки` не нужно раскладывать на сериализацию, доставку задачи, очередь Piscina и десериализацию. В первой версии это один честный этап roundtrip: главный поток отправляет задачи, ждет `pool.run(...)`, получает массив результатов и переходит к слиянию. Worker-подшаги с дополнительным отступом показывают, какая часть этого ожидания была реальной работой внутри worker.
...
- строки верхнего уровня (`Подготовка YAML-проекта`, `Первичная проверка YAML`, `Проверка зависимостей`) объединяют main- и worker-замеры соответствующего этапа;
