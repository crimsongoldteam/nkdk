...
- статус импорта;
- модель;
- построенный `fieldIndex` для владельцев;
- ошибки, которые делают объект непригодным как владелец.

На первом шаге таблица хранит и модель, и `fieldIndex`. Это нужно, потому что существующая проверка `ПутьКДанным` использует не только индекс реквизитов, но и модель: типы определяемых типов, типы констант, состав общих реквизитов и значения перечислений. Если позже измерения покажут слишком большой расход памяти, модель можно заменить более узкими сериализуемыми индексами, но это отдельная оптимизация.

Для формы первый проход означает: прочитать и разобрать `Форма.yaml`, проверить schema, импортировать модель формы, построить индекс реквизитов и элементов формы, собрать найденные `ПутьКДанным`. Проверки, которым достаточно только формы, выполняются здесь.
...
Для формы второй проход означает: проверить собранные `ПутьКДанным` относительно владельца и проекта. Например, `ПутьКДанным: Объект.Товар` проверяется только после того, как в таблице объектов есть `Свойства.yaml` владельца и связанные типы, нужные для прохода по цепочке.

`ProjectMetadataResolver` и `OwnerMetadataCache` в параллельной validation должны получить новый вариант договора: работать по таблице объектов и списку известных файлов, а не напрямую через `existsSync` и `ProjectYamlCache`. В частичном режиме запрос отсутствующей YAML-зависимости сначала возвращает результат `needsDependency` с путём файла, если путь можно определить нейтральными правилами проекта. После дозагрузки второй проход повторяется. Диагностическая ошибка "не найдено" формируется только если путь нельзя определить или файл отсутствует после попытки дозагрузки.
