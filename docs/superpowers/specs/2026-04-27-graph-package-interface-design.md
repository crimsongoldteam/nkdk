...
1. **Что хранить в `props` узла.** Решено: координаты — `name`, `filePath`, `line`, `col` (не `offset` — он не нужен живым потребителям, line:col удобнее и для VSCode, и для сообщений об ошибках). Смысловые вложенные структуры из JS-модели (типы, описания, ссылки) выносятся как **рёбра** к отдельным узлам, а не сериализуются в свойства. Простые примитивные поля JS-модели (булевые флаги, енумы как строки) — добавляются на узел **по мере появления Cypher-запросов**, которые их читают, а не все сразу. Никакого `data: string` с JSON. Маркер «узел разрешён» (был `resolved` в текущем `update-graph`) — см. вопрос 3.

2. **Многофайловые логические сущности.** Решено: один узел = один файл, межфайловые связи делаются через рёбра, а не через массив `filePaths`. Для формы это значит: элемент формы — отдельный узел в `Форма.nkdk` (идентичность, структура), его свойства — **отдельные `:Property`-узлы в `Форма.yaml`**, по одному на каждое значение. Связь — ребро `(:FormElement)-[:HAS_PROPERTY]->(:Property)`. Преимущества: каждый узел имеет точные `line/col` своего файла; инвалидация любого файла затрагивает только его узлы; Cypher-фильтры по свойствам работают через метку `:Property`.
