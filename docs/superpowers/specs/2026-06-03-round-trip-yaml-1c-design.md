...
## Наблюдения по диагностике `all`

На `/home/nikita/git/round-trip/all` после исправления ранних блокеров загрузка дошла до ошибки формы:

```text
CommonForms/ДинамическийСписок/Ext/Form.xml, invalid data path: "СОсновнойТаблицей.ЭтоГруппа"
```

Подтверждённые причины предыдущих ошибок:

- порядок внутри `<Attribute>` важен: `<Type>` должен идти перед `<Settings xsi:type="DynamicList">`, иначе 1С не сопоставляет `DynamicList`;
- для форм с `QueryText` внешний `.query` должен сохраняться при `XML -> YAML` и читаться при `YAML -> XML` без reference; потеря `QueryText` даёт ошибки вида `invalid data path` для полей динамического списка.

Проверенная отрицательная гипотеза:

- ручное добавление в сгенерированный XML потерянных `dcssch:value xsi:nil`, `dcsset:userSettingID`, `dcsset:itemsUserSettingID` и строковых `presentation/userSettingPresentation` не исправляет `invalid data path: "СОсновнойТаблицей.ЭтоГруппа"`;
- значит, отсутствие этих конкретных полей не является причиной текущей ошибки.

Разделительная проверка выполнена: загрузка исходного XML `/home/nikita/git/round-trip/all` в чистую файловую базу 1С падает с той же ошибкой:

```text
CommonForms/ДинамическийСписок/Ext/Form.xml, invalid data path: "СОсновнойТаблицей.ЭтоГруппа"
```

Вывод: эта конкретная ошибка не является регрессией `XML -> YAML -> XML` без reference. Для `round-trip-yaml-1c` её нужно либо отмечать как известную невалидность исходного каталога на текущей платформе, либо проверять следующий блокер на подмножестве/после исключения этой формы.
