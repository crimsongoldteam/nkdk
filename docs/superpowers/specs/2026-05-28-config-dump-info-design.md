...
Для существующей записи берём `id` и, если он есть у этой записи, `configVersion` из `referenceDir/ConfigDumpInfo.xml`. Для новой версионируемой записи генерируем:

- `id` - UUID;
- `configVersion` - 40 hex-символов, как в реальных выгрузках 1С.

Для новой неверсионируемой дочерней записи генерируем только `id`. По XSD `configVersion` отсутствует у неверсионируемых подчинённых объектов; в реальных выгрузках это видно на дочерних `Metadata` внутри верхнеуровневого объекта.
...
### Переименование Верхнего Объекта

Для переименования верхнего объекта переносится вся ветка владельца.

Пример:

```text
Справочник.Товары -> Справочник.Номенклатура
Catalog.Товары -> Catalog.Номенклатура
```

Итог:

- запись `Catalog.Товары` не пишется;
- запись `Catalog.Номенклатура` получает старые `id` и `configVersion`;
- дочерние записи `Catalog.Товары.Attribute.*`, `Catalog.Товары.TabularSection.*` и похожие записи владельца переписываются с новым префиксом `Catalog.Номенклатура.*`;
- если у дочерней записи нет отдельной миграции, её локальное имя сохраняется, а `id` ищется по старому полному имени.

### Переименование Дочернего Элемента

Для переименования дочернего элемента переносится только эта запись.

Пример:

```text
Справочник.Номенклатура.Реквизит.Артикул -> Справочник.Номенклатура.Реквизит.КодАртикула
Catalog.Номенклатура.Attribute.Артикул -> Catalog.Номенклатура.Attribute.КодАртикула
```

Итог:

- старая дочерняя запись не пишется;
- новая дочерняя запись получает старый `id`;
- `configVersion` для такой записи не пишется, если его не было у reference-записи;
- если одновременно переименованы родитель и ребёнок, сначала применяется путь родителя, затем локальное имя ребёнка.

### Удаление

Удаление работает от целевого YAML-состояния и применённых миграций.

- Удалённый верхний объект удаляет всю ветку `ConfigDumpInfo`: саму запись объекта, дочерние записи и внешние записи с этим префиксом.
- Удалённый дочерний элемент не попадает в дочерний список владельца.
- Миграционный сценарий `Удалить` + `Добавить` для того же пути считается пересозданием: новая запись получает новый `id`, потому что у неё нет `referencePath`.
- Простое отсутствие элемента в YAML без конфликтной пары `удалено + добавлено` тоже означает удаление из итогового `ConfigDumpInfo.xml`.

### Внешние Записи Из Reference

Для записей, которые первая версия не выводит из YAML-модели напрямую, действует консервативное правило сохранения.

- Если владелец жив и не переименован, reference-запись сохраняется как есть.
- Если владелец переименован, префикс записи переносится вместе с владельцем.
- Если владелец удалён, reference-запись удаляется.
- Новые внешние записи для форм, модулей, макетов и похожих файлов в первой версии не создаются. Это отдельное расширение, которое лучше привязать к `xmlManifest`, потому что именно manifest знает, какие внешние XML/BSL-файлы реально записал экспорт.
...
Существующие записи reference-файла для внешних XML/BSL-частей, форм, команд, макетов, справки и других версионируемых файлов сохраняются, если их владелец остаётся в целевой конфигурации, с учётом переноса префикса при переименовании владельца. Новые записи для таких внешних файлов отдельно не выводятся в этой задаче.
...
5. Сохранить существующие `id/configVersion` или сгенерировать новые значения по типу записи.
