...
- появляется `metadata/register.ts` с функцией `registerCoreMetadata()`;
- `registerCoreMetadata()` вызывает `registerCommonObjects()`, `registerForms()` и `registerAppliedObjects()` в явном порядке;
- каждая функция регистрации идемпотентна: повторный вызов не меняет поведение и не дублирует записи;
- `metadata/commonObjects/index.ts`, `metadata/forms/index.ts` и `metadata/appliedObjects/index.ts` временно могут сохранить побочный запуск ради совместимости, но рядом появляется явная функция регистрации;
- `packages/core/index.ts` экспортирует `registerCoreMetadata()` и на переходный период сам вызывает её, чтобы не ломать текущих потребителей;
- внутренний код, тесты, CLI и расширение постепенно переходят с `import "~/metadata/appliedObjects"` на явный `registerCoreMetadata()` или более узкую функцию регистрации области;
- после перехода побочный вызов из `packages/core/index.ts` можно будет удалить отдельным несовместимым изменением, если это будет нужно.

Порядок регистрации фиксируется явно:

1. `registerCommonObjects()` — базовые property-типы и правила коллекций metadata item.
2. `registerForms()` — формовые common objects и элементы формы.
3. `registerAppliedObjects()` — верхнеуровневые applied objects.

Такой порядок отражает фактические зависимости: forms используют common objects, applied objects используют и common objects, и forms.

Нельзя пытаться сразу заменить 431 побочный `import "./..."` на ручной список вызовов в одном изменении. Первый шаг — обернуть существующие побочные входы функциями регистрации и зафиксировать контракт. Затем отдельными срезами можно переносить конкретные регистрации из побочного выполнения верхнего уровня в явные `register...()` рядом с владельцем.

Тесты для этого среза:

- `registerCoreMetadata()` можно вызвать дважды, и базовые сценарии XML/YAML/JSON Schema продолжают работать;
- публичные функции `@nakidka/core`, CLI и расширение не требуют от потребителя ручной регистрации на переходном этапе;
- новый внутренний тестовый вход может очищать реестры и заново вызывать `registerCoreMetadata()` без зависимости от `tests/setupTests.ts`;
- проверка границ запрещает новые прямые импорты `~/metadata/appliedObjects`, `~/metadata/commonObjects`, `~/metadata/forms` только ради регистрации, если есть явная функция.
