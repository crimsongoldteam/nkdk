...
значения базовой формы. У собственного элемента отсутствие YAML-ключа должно
...
- использовать один семантический механизм в XML → YAML, YAML → XML,
  validation, поиске ссылок и переименовании;
...
Для заимствованной формы используется основной реквизит базовой формы, если
расширение не объявляет собственного. Встроенный XML `BaseForm` не обязан
содержать `MainAttribute`: источником истины служит YAML базовой формы, уже
доступный операциям над расширением.

Если основного реквизита нет, неявный путь обычного элемента не вычисляется.

### Происхождение элемента

Элемент считается заимствованным, если элемент с тем же именем присутствует в
дереве базовой формы. Имена элементов формы уникальны в пределах формы, поэтому
отдельный служебный признак в YAML не нужен.

Элемент, отсутствующий в базовой форме, считается собственным, в том числе если
он добавлен расширением внутрь заимствованной группы или таблицы.
...
| Заимствованный элемент | ключ отсутствует | наследовать точное значение базового элемента |
| Заимствованный элемент | `ПутьКДанным: ""` | ошибка validation |
| Заимствованный элемент | явный путь | переопределить значение базового элемента |
...
Подготовитель становится общей точкой входа для import, export, validation и
операций. Узкий `collectFormTabularElementsFromYAML`, выполняющий отдельный обход
...
- ближайшую таблицу;
- наличие элемента в заранее построенной карте базовой формы.
...
Операции над расширением уже получают `baseYaml` для построения XML `BaseForm`.
Тот же объект передаётся подготовителю основной части формы. Сейчас
`convertClientApplicationFormYAMLToXMLCore` получает только сформированный
`baseFormXML`. Внешняя граница формы один раз вызывает подготовитель с `baseYaml`
и передаёт в core готовый `formDataPathContext`; общий runtime правил не узнаёт о
заимствованных формах.

Подготовитель сначала строит компактную карту базовых элементов и их эффективных
путей, затем одним проходом обрабатывает текущую форму. Эта карта также
переиспользуется проекцией `BaseForm`, чтобы не индексировать одни элементы
повторно.

Импорт расширения использует базовую конфигурацию, которую координатор уже
требует для расширения. Изолированный импорт расширения без базовой конфигурации
не вводит приблизительную семантику происхождения.
...
flowchart TD
    ...
    C -->|Нет| D["Наследовать DataPath базового элемента"]
...
2. Для расширения координатор предоставляет YAML базовой формы.
3. Единый подготовитель строит индекс и кандидаты в памяти без повторного чтения
   или разбора файлов.
4. Уже существующая финализация отложенных `DataPath` преобразует стандартные
...
6. У заимствованного элемента отсутствующий XML-тег опускается как наследование.
7. Остальные пути сохраняются явно.

### YAML → XML

1. Подготовитель вызывается один раз до преобразования свойств.
2. Для отсутствующего ключа собственного элемента преобразователь получает
   мемоизированный кандидат.
3. Пустая строка собственного элемента подавляет XML-тег.
4. Отсутствующий ключ заимствованного элемента не создаёт XML-тег и оставляет
   наследование платформе.
5. Явный нестандартный путь проходит через действующий resolver и сериализуется
   обычным механизмом.

### Validation и операции

Validation, поиск ссылок, переименование и удаление используют тот же
подготовленный контекст и те же канонические результаты resolver. Они не
разбирают имена элементов и таблиц повторно.
...
- не более одного обхода компактной карты базовой формы;
...
Диагностика указывает путь YAML к свойству и вычисленное каноническое значение.
Сравнение выполняется по internal-представлению общего resolver, поэтому
`Наименование` и `Description` не считаются разными путями.
...
- отсутствующий путь заимствованного элемента наследуется из базовой формы;
- наследуется нестандартный путь, а не заново вычисленный кандидат;
- наследуется отсутствие пути базового элемента;
...
- собственная колонка внутри заимствованной таблицы использует эффективный путь
  этой таблицы;
- явный путь заимствованного элемента переопределяет базовый;
...
- поиск ссылок и переименование видят восстановленный эффективный путь;
- вспомогательные пути и `Данные` кнопок не изменяются;
- один подготовитель используется import, export, validation и операциями.
