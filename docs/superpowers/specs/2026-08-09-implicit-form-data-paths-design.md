...
Для заимствованной формы используется основной реквизит текущей формы основной
конфигурации, если расширение не объявляет собственного. Встроенный XML
`BaseForm` и сохранённая `БазоваяФорма.yaml` описывают историческое состояние
заимствованной формы и не обязаны содержать признак основного реквизита. Поэтому
они определяют происхождение и унаследованный путь элементов, а текущая форма
`cf` — недостающий основной реквизит.
...
`inheritedYaml` задаёт историческое состояние, от которого наследуются
элементы, а `currentBaseYaml` — резервный источник основного реквизита. Для
обычной формы оба аргумента отсутствуют.
...
Переиспользуется существующий договор базовых форм, введённый в `develop`:

- при XML-import историческим источником является фактический встроенный
  `BaseForm`, уже импортированный в кандидат `БазоваяФорма.yaml`;
- при YAML → XML историческим источником является сохранённая
  `БазоваяФорма.yaml`, а при её отсутствии — проекция текущей формы `cf`;
- текущая форма `cf` остаётся отдельным источником основного реквизита.

Это два разных назначения данных, поэтому сохранённая основа не подменяется
текущей формой: из неё наследуется точный исторический `DataPath`, включая
нестандартный путь или отсутствие пути. Текущая форма используется только как
резервный источник признака основного реквизита.

Внешняя граница формы один раз собирает `formDataPathContext` из рабочей формы,
выбранного исторического источника и текущей формы `cf`, затем передаёт готовый
...
путей, затем одним проходом обрабатывает текущую форму. Существующие чтение,
проверка хэша и выбор источника базовой формы не дублируются.
...
2. Для расширения импортируется встроенный `BaseForm`, затем до сериализации
   рабочей формы читается уже требуемая текущая форма `cf`.
3. Отложенные значения встроенной основы финализируются первыми; после этого
   единый подготовитель строит индекс и кандидаты в памяти без повторного чтения
   или разбора файлов.
...
1. Существующий `BaseFormSource` выбирает сохранённую основу либо текущую форму
   `cf`; текущая форма передаётся отдельно, если выбрана сохранённая основа.
...
- источники базовой формы читаются существующим `BaseFormSource` и XML-import
  worker, без дополнительного обращения к диску;
