...
`metadataDocument/rules.ts:uuid` имел `xmlParents: ["Document"]`, что вынуждало движок класть uuid внутрь вложенного элемента `<Document uuid="…"/>` после Properties. У Catalog в аналогичном правиле `xmlParents: []` — uuid идёт атрибутом на корневой элемент. Замена `["Document"]` → `[]` сразу закрыла оба наблюдавшихся симптома: пустой корневой `<Document>` и лишний внутренний `<Document uuid="11111111…"/>`. На `minimal.xml` тест теперь зелёный.
