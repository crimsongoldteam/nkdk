...
Импорт читает каждый XML-файл с диска один раз. Первый проход строит полный семантический индекс, индекс зависимостей и ожидающие проверки непосредственно из XML, а затем сохраняет разобранный XML в компактном двоичном виде. Второй проход на том же worker восстанавливает XML-объект, строит окончательный YAML с готовыми межфайловыми решениями, выполняет контрольный toXML, один раз записывает YAML и сразу освобождает данные задания.
...
flowchart TD
  ...
  merge["Собрать глобальные индексы,<br/>ссылки и ожидающие проверки"]
  decisions["Классифицировать межфайловые решения<br/>и подготовить профиль восстановления XML"]
...
- ссылки, зависимости и ожидающие проверки, включая `fillValue` и пути форм;
...
Эквивалентность фактов проверяется не против предварительного состояния нового импорта, а против семантического вклада, который старый импорт получает после построения полного YAML. Для одного и того же задания должны совпадать цели, владельцы, поля, формы, логические адреса, ссылки, зависимости и ожидающие проверки. Режим не становится новым набором предметных правил: это отдельный способ выполнения существующего обхода и общих анализаторов зависимых значений.

Первый проход не должен ни терять факты, которые появляются при проверке полного YAML, ни добавлять проверки, которых старый импорт для этого metadata item не выполняет. До доказанной эквивалентности координатор не использует предварительный вклад для классификации `!xml/invalid` и `!xml/important`.
...
- ссылки на вложенные стандартные реквизиты, включая `ТабличнаяЧасть.…СтандартныйРеквизит.Ссылка`, разрешаются по индексу первого прохода и не получают лишний `!xml/invalid`;
- для реквизита с `DefinedType` первый проход сохраняет проверку `fillValue`, а несовместимое значение получает тот же `!xml/invalid`, что в старом импорте.
...
- равенство полного facts-only вклада и вклада старого полного YAML: targets, owners, fields, forms, logical addresses, pending references, pending checks и dependencies;
- отсутствие проверки самостоятельной формы у вложенного `MetadataCommonForm`;
- разрешение ссылки на стандартный реквизит вложенной табличной части из facts-only индекса;
- сохранение `fillValue` pending check для реквизита с `DefinedType`;
