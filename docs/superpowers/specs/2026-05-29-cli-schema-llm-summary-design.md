...
Если запрос совпал с одним полем по точному имени, `--search` всё равно остаётся режимом поиска; за подробной сводкой нужно вызвать `--prop <key>`.
...
- `--keys` сочетается с `--required`, `--prop`, `--search`;
