...
```text
config load-files --dir="<каталог>" --archive="package.zip" --no-check --list-file="<каталог>/load.lst" --partial --update-config-dump-info
```
...
Рядом с копией ZIP сессия записывает `load.lst` из плана загрузки. ZIP также
содержит этот список по существующему договору частичного пакета, но агент
Конфигуратора читает отдельный файл.

Параметры `--dir`, `--archive`, `--list-file` и `--partial` соответствуют
...
Точная форма команды и расположение отдельного `load.lst` подтверждены журналом
EDT и сквозным экспериментом на платформе 8.3.27.2214. ZIP не распаковывается.
...
3. копирует ZIP без распаковки и записывает рядом `load.lst`;
4. захватывает позицию `/Out`-журнала Конфигуратора;
5. выполняет `load-files`;
...
- штатный отказ `load-files`: платформа подтвердила неуспех, пакет
...
| `@nkdk/platform` | Сессия агента, служебные копии ZIP и списка загрузки, команда 1С, результат передачи и `platform.log` |
...
Сначала, после минимальной реализации построителя команды и служебного каталога,
на копии базы подтверждается, что агент читает отдельный `load.lst` рядом с ZIP.
До этого результата не реализуются состояния доставки и публичный MCP-координатор.
