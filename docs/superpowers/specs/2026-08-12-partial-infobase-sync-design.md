...
После реализации обновляется `.agents/architecture.md`. До реализации файл не
меняется: он является монитором текущего кода, а не описанием будущего решения.

В архитектурном мониторе изменения выражаются таблицей и схемой, без переноса
подробного договора из этой спецификации.

Таблица ответственности должна зафиксировать:
...
Схема операции заменяет текущий узел «Вне NKDK: загрузить пакет в 1С» и
показывает границы пакетов и устойчивые состояния:
...
Заголовок операции и колонка переиспользования меняются с «Частичная
синхронизация YAML → XML» на «Частичная синхронизация с информационной базой».
В термины добавляется «Ожидающее состояние синхронизации»: ZIP,
снимок-кандидат и устойчивая фаза доставки, не допускающая опасного повтора.
