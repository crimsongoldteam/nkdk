...
- [ ] **Step 5: Выполнить mutation testing изменённых диапазонов**

  Следовать `.agents/testing.md`; для каждого изменённого пакета запустить настроенную mutation-команду только по затронутым production-файлам. Содержательные выжившие мутанты закрыть усилением существующих `it.each` либо уникальным новым тестом.
