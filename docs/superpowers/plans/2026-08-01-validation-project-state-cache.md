...
- Внутри задачи объединять связанные договоры в один TDD-блок. Для блока сначала запускать один набор RED-тестов, во время реализации повторять только быстрые целевые тесты, а после общего GREEN выполнять один узкий mutation-прогон готового блока по правилам `.agents/testing.md`.
...
- Широкий mutation-прогон всех production-изменений выполнить один раз в Task 15.
...
## Task 8: Перевести публичную validation и удалить экспериментальные пути
...
- [ ] **Step 4: Удалить второй shared-graph проход из prepared pool**
  ...
  Удалить API, передающие `SharedValidationGraph`/`SharedValidationSnapshot` для validation. Сохранить только локальную обработку выбранных YAML и writer producer. Не удалять shared-типы, пока Task 10/11 не переведут sync/import; пометить их внутренними legacy-экспортами без новых потребителей.
...
## Task 9: Перевести поиск ссылок и переименование
...
- [ ] **Step 4: Перевести поиск на read session**
  ...
  После refresh разрешить цель и получить обращения пакетными методами store. Не читать все YAML и не собирать metadata-модели. Если поиск продолжен при error diagnostics, добавить `search_result_may_be_incomplete`, не заменяя исходные ошибки. `allowWrite` не влияет на поиск; оставить его только на MCP-границе для совместимости до Task 12, затем удалить из core-параметров поиска.
...
## Task 10: Перевести полную и частичную sync на обязательную актуализацию
...
## Task 11: Интегрировать import без замедления ранней записи
...
## Task 12: Подключить единый жизненный цикл и команды MCP
...
## Task 13: Удалить старое полное представление индекса
...
## Task 14: Актуализировать архитектурные документы и ограничения
...
## Task 15: Выполнить приёмочное профилирование и полную проверку
...
- [ ] **Step 5: Выполнить единый широкий mutation-прогон реализации**

  Определить все production-файлы, изменённые реализацией от `e768ba6321fc99b2623e04f1fe72a06c77f07b38` до `HEAD`, и выполнить mutation testing по полным изменённым файлам. Для управляемого времени и памяти разрешается разбить прогон на воспроизводимые пакеты, но не сужать цели до выборочных строк. Отчёты всех пакетов вместе должны покрывать весь список изменённых production-файлов. Содержательные выжившие мутанты закрыть усилением существующих `it.each` либо уникальным новым тестом. Отчёт не должен содержать `Timeout`, `RuntimeError` или `CompileError`.

- [ ] **Step 6: Выполнить окончательную проверку**
...
- [ ] **Step 8: Commit результата профиля**
