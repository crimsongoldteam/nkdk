...
**Архитектура:** Существующий `prepareForDump` одновременно строит значение для `js-yaml` и смысловые данные: пустой объект остаётся `{}` в обоих представлениях. После dump один линейный проход меняет только полные строки пустого отображения. Уже существующий нормализующий проход parser восстанавливает неявное пустое значение как `{}`, поэтому JSON Schema не расширяется.

> Коррекция после контрольного импорта: первоначальные шаги задачи 2 с `undefined` отменены. Полный импорт показал, что такая форма ломает общие преобразователи пустых объектов; итоговый договор реализован в parser без нового обхода данных.
...
### Задача 2: Чтение пустых тел именованных metadata-коллекций (первоначальный вариант отменён)

Шаги ниже сохранены как история первоначального плана и в итоговую реализацию
не входят. Вместо расширения схем `undefined` parser восстанавливает `{}` на
уже существующем нормализующем проходе.
