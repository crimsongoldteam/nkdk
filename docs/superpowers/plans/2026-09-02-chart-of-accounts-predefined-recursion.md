    ...
    ОсновныеСредства: {
      Элементы: {
        ОСвОрганизации: {
          ...
          Порядок: "01.01",
          ПризнакиУчета: {
            "ChartOfAccounts.Хозрасчетный.AccountingFlag.Налоговый": true,
          },
          ВидыСубконто: {
...
- [ ] **Шаг 4: Переключить корневую и вложенную коллекции на конкретный тип**
  ...
  childItems: chartOfAccountsPredefinedItemCollectionRule({
    ...PredefinedItemRules.properties.childItems,
  }),
  ```
...
- [ ] **Шаг 1: Добавить падающую проверку logicalAddress вложенного счёта**
  ...
  Перед добавлением production-кода этот тест должен падать, потому что вложенная общая коллекция не формирует договор конкретного типа плана счетов.
...
- [ ] **Шаг 3: Добавить проверку рекурсивной JSON Schema**
  ...
  Экспортировать JSON Schema для `ChartOfAccountsPredefinedRules.properties.items` реальным реестром композиции и скомпилировать её. Проверить, что схема принимает вложенный счёт со специфичными полями и отклоняет неизвестное поле:

  ```ts
  expect(check.Check({
    ОсновныеСредства: {
      Элементы: {
        ОСвОрганизации: { ВидСчета: "Активный", Забалансовый: true },
      },
    },
  })).toBe(true)
  expect(check.Check({
    ОсновныеСредства: { Элементы: { ОСвОрганизации: { Неизвестное: true } } },
  })).toBe(false)
  ```
