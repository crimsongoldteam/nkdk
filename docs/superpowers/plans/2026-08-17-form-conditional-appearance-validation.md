...
- Для верхнеуровневых `Поля` проверяется существование элемента формы, но не
  ограничивается его вид; `Поля` оформления `DynamicList` разрешаются как
  поля набора данных относительно реквизита списка.
...
- [x] **Step 1: Добавить падающий тест обхода двух корней**
  ...
  Проверить YAML-пути, рекурсивные группы и `tableContext: { dataPath: "Список" }`
  для операндов и оформляемых полей `DynamicList`.
...
- [x] **Step 7: Реализовать проверку целей и операндов**
  ...
  Верхнеуровневые цели искать в `dataPathContext.elementsByName`, а цели
  `DynamicList` разрешать относительно реквизита списка через общий resolver.
  Неизвестное обычное имя — ошибка на точном YAML-пути. Статусы `unavailable`, `deferred` и `error` у
...
- [x] **Step 2: Реализовать импортный завершающий проход**
  ...
  Для цели использовать `xml/reference`. Верхнеуровневую цель проверять по
  эффективному индексу элементов, а цель `DynamicList` — общим resolver
  относительно реквизита списка. Тегировать `unavailable`, `deferred`,
  `error`; предупреждений не создавать. Расширить
  `finalizeImportedFormDataPaths` в `worker.ts`, не вводить новый проход.
...
- [x] **Step 1: Обновить restrictions**
  ...
  ```md
  - Resolver может не определять часть полей набора данных DynamicList; при
    XML-импорте операнды получают !xml/value, оформляемые поля —
    !xml/reference и не проходят смысловую проверку.
