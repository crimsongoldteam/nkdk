...
export function analyzeTestDurationReport(report, lifecycleReport, environment = {}) {
  ...
  const integration = environment.NKDK_TEST_SUITE === "integration"
  ...
  const fileDurationLimit = (integration
    ? INTEGRATION_TEST_FILE_LIMIT_MS
    : TEST_FILE_LIMIT_MS) * limitMultiplier
  ...
  for (const suite of report.testResults) {
    ...
    if (fileDuration > fileDurationLimit) {
...
export function parseArguments(argv) {
  ...
  if (args.length !== 4 || args[0] !== "--report" || args[1] === "" ||
    ...
    throw new Error("Использование: node packages/rules/scripts/assert-test-durations.mjs --report <path> --lifecycle-report <path>")
  }
