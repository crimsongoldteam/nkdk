...
function cleanFormFixtureXmlFile(inputFile: string, outputFile: string): void {
  const xmlContent = readFileSync(inputFile, "utf-8")
  const fileName = inputFile.split(/[/\\]/).pop() || ""
  const processedXml = cleanFormFixture(xmlContent)
  const outputDir = dirname(outputFile)
  mkdirSync(outputDir, { recursive: true })
  writeFileSync(outputFile, processedXml, "utf-8")
}
...
export function cleanFormFixturesXmlFiles(inputPath: string, outputPath: string): void {
  try {
    if (isFile(inputPath)) {
      const progressBar = new cliProgress.SingleBar(
        {
      ...
      try {
        if (isDirectory(outputPath)) {
          const outputFile = join(outputPath, fileName)
          cleanFormFixtureXmlFile(inputPath, outputFile)
        } else {
          cleanFormFixtureXmlFile(inputPath, outputPath)
        }
        progressBar.update(1, { file: fileName })
      } finally {
        progressBar.stop()
      ...
      processDirectoryFormFixturesXml(inputPath, outputPath)
      console.log(`\n✓ Обработка завершена: ${inputPath} -> ${outputPath}`)
    } else {
      console.error(`Ошибка: ${inputPath} не является файлом или каталогом`)
      process.exit(1)
...
function processDirectoryFormFixturesXml(inputDir: string, outputDir: string): void {
  mkdirSync(outputDir, { recursive: true })
  const xmlFiles = collectXmlFiles(inputDir, inputDir)
  ...
  try {
    for (let i = 0; i < xmlFiles.length; i++) {
      ...
      try {
        cleanFormFixtureXmlFile(file.input, outputFile)
        processedCount++
      } catch (error) {
        let errorMessage = "Неизвестная ошибка"
        if (error instanceof Error) {
