...
export function readExplicitElementXMLName(yaml: unknown): string | undefined {
  ...
  if (yamlScalarTagAt(item, "Имя") !== "xml/name" || typeof item.Имя !== "string") {
    throw new Error("Поле Имя встроенного элемента допустимо только с тегом !xml/name")
  }
  return xmlAnomalyTagPayload("xml/name", item.Имя)
}

export function writeExplicitElementXMLName(yaml: Record<string, unknown>, value: string): void {
  yaml.Имя = xmlAnomalyTagValue("xml/name", value)
