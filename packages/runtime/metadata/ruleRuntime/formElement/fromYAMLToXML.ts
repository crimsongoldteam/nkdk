...
import { getFormElementCollectionPropertyTypes, getFormElementCollectionTypes } from "./registry"
...
export function registerDirectFormElementCollections(): void {
  for (const propertyType of getFormElementCollectionPropertyTypes()) {
    const elementTypes = getFormElementCollectionTypes(propertyType) ?? []
    const fallbackRule = getElementRule(elementTypes[0])
...
export function resolveFormElementRule(params: {
  ...
  const allowedTypes =
    getFormElementCollectionTypes(params.propertyRule.type) ?? getFormElementCollectionTypes("GroupChildItems") ?? []
