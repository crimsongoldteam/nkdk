import {
  InputField,
  InputFieldPartialEnterprise,
  InputFieldPreview,
  InputFieldTypedEnterprise,
} from "~/metadata/forms/elements/inputField/types"
import { IFormatElementResult } from "~/metadata/forms/format/types"
import { FormElementType } from "~/metadata/metadataFactory/types"
import { RequiredFieldsElement } from "~/tests/types"

export const fullInputField: RequiredFieldsElement<InputField> = {
  elementType: FormElementType.InputField,
  name: "ПолеВвода",
  autoCellHeight: true,
  cellHyperlink: true,
  dataPath: "Объект.Реквизит",
  defaultItem: true,
  displayImportance: "High",
  editMode: "EnterOnInput",
  enabled: true,
  fixingInTable: "None",
  footerBackColor: { type: "WebColor", value: "White" },
  footerDataPath: "Объект.РеквизитПодвала",
  footerFont: { kind: "StyleItem", ref: "NormalTextFont" },
  footerHorizontalAlign: "Left",
  footerPicture: {
    type: "StandardPicture",
    ref: "Print",
    loadTransparent: true,
  },
  footerText: {
    items: { ru: "Текст подвала" },
  },
  footerTextColor: { type: "WebColor", value: "Black" },
  headerHorizontalAlign: "Left",
  headerPicture: {
    type: "StandardPicture",
    ref: "Print",
    loadTransparent: true,
  },
  horizontalAlign: "Left",
  horizontalAlignInGroup: "Left",
  readOnly: false,
  shortcut: "Ctrl+S",
  showInFooter: true,
  showInHeader: true,
  skipOnInput: false,
  titleBackColor: { type: "WebColor", value: "Blue" },
  titleFont: { kind: "StyleItem", ref: "NormalTextFont" },
  titleHeight: 20,
  titleLocation: "Left",
  titleTextColor: { type: "WebColor", value: "Black" },
  toolTip: {
    items: { ru: "Подсказка" },
  },
  toolTipRepresentation: "None",
  type: "InputField",
  verticalAlign: "Top",
  verticalAlignInGroup: "Top",
  visible: true,
  warningOnEdit: {
    items: { ru: "Предупреждение" },
  },
  warningOnEditRepresentation: "DontShow",
  contextMenu: { autofill: false, childItems: [] },
  extendedTooltip: { title: { items: { ru: "Расширенная подсказка" }, formatted: false } },
  table: "Таблица",
  typeRestriction: { type: ["string"] },
  allowInputEmptyMultipleValues: true,
  allowMultipleValuesDuplicates: false,
  autoCapitalizationOnTextInput: "AllCharacters",
  autoChoiceIncomplete: true,
  autoCorrectionOnTextInput: "Use",
  autoFillHint: "Email",
  autoMarkIncomplete: true,
  autoMaxHeight: true,
  autoMaxWidth: true,
  autoShowClearButton: "Always",
  autoShowOpenButton: "FilledOnly",
  availableTypes: { type: ["DocumentRef.КакойТоДокумент"] },
  backColor: { type: "WebColor", value: "Blue" },
  borderColor: { type: "WebColor", value: "Green" },
  choiceButton: true,
  choiceButtonPicture: {
    type: "StandardPicture",
    ref: "Print",
    loadTransparent: true,
  },
  choiceButtonRepresentation: "ShowInInputField",
  choiceFoldersAndItems: "FoldersAndItems",
  choiceForm: "Catalog.Справочник.Form.ФормаВыбора",
  choiceHistoryOnInput: "DontUse",
  choiceList: [
    {
      type: "formChoiceListDesTimeValue",
      presentation: { items: { ru: "Значение 1" } },
      value: {
        type: "string",
        value: "Значение1",
      },
    },
  ],
  choiceListButton: true,
  choiceListHeight: 200,
  choiceParameterLinks: [
    {
      name: "Отбор.Владелец",
      dataPath: "Catalog.Справочник.Attribute.Реквизит",
      valueChange: "Clear",
    },
  ],
  choiceParameters: [
    {
      name: "Отбор.Параметр",
      value: {
        type: "string",
        value: "Значение",
      },
    },
  ],
  chooseType: true,
  clearButton: true,
  createButton: true,
  dropListButton: true,
  dropListWidth: 300,
  editFormat: { items: { ru: "Формат редактирования" } },
  editText: "Текст редактирования",
  editTextUpdate: "OnValueChange",
  extendedEdit: true,
  font: { kind: "StyleItem", ref: "NormalTextFont" },
  format: { items: { ru: "Формат отображения" } },
  height: 100,
  heightControlVariant: "UseContentHeight",
  horizontalStretch: true,
  incompleteChoiceMode: "OnActivate",
  inputHint: { items: { ru: "Подсказка ввода" } },
  listChoiceMode: true,
  markIncomplete: true,
  markNegatives: true,
  mask: "999",
  maxHeight: 500,
  maxValue: 100,
  maxWidth: 400,
  minValue: 0,
  multiLine: true,
  multipleValuePictureDataPath: "Объект.Picture",
  multipleValuePictureShape: "Circle",
  multipleValuePictureSize: "Large",
  multipleValuePresentationDataPath: "Объект.Presentation",
  multipleValuesBackColor: { type: "WebColor", value: "Magenta" },
  multipleValuesExtendedEdit: true,
  multipleValuesFont: { kind: "StyleItem", ref: "NormalTextFont" },
  multipleValuesHyperlink: true,
  multipleValuesPicture: {
    type: "CommonPicture",
    ref: "ОбщаяКартинка",
    loadTransparent: false,
  },
  multipleValuesTextColor: { type: "WebColor", value: "Red" },
  multipleValueValueDataPath: "Объект.Value",
  onScreenKeyboardReturnKeyText: "Done",
  openButton: true,
  passwordMode: true,
  quickChoice: true,
  selectedText: "Выделенный текст",
  showCheckBoxesInDropListWhenInputMultipleValues: true,
  specialTextInputMode: "Email",
  spellCheckingOnTextInput: "Use",
  spinButton: true,
  textColor: { type: "WebColor", value: "Yellow" },
  textEdit: true,
  typeDomainEnabled: true,
  typeLink: {
    dataPath: "Catalog.КакойТоСправочник.Attribute.КакойТоРеквизит",
    linkItem: 1,
  },
  verticalStretch: true,
  width: 200,
  wrap: true,
  userVisible: {
    common: true,
    values: [{ name: "Администратор", value: true }],
  },
  title: {
    items: { ru: "Поле ввода" },
  },
  events: {
    onChange: "ПроцедураПриИзменении",
    autoComplete: "ПроцедураАвтоПодбора",
    multipleValuesAdd: "ПроцедураДобавленияМножественныхЗначений",
    editTextChange: "ПроцедураИзмененияТекстаРедактирования",
    startChoice: "ПроцедураНачалаВыбора",
    startListChoice: "ПроцедураНачалаВыбораИзСписка",
    choiceProcessing: "ПроцедураОбработкиВыбора",
    multipleValueURLProcessing: "ПроцедураОбработкиНавигационнойСсылкиМножественногоЗначения",
    commandGenerateProcessing: "ПроцедураОбработкиФормированияКоманд",
    textEditEnd: "ПроцедураОкончанияВводаТекста",
    opening: "ПроцедураОткрытия",
    multipleValueOpening: "ПроцедураОткрытияМножественногоЗначения",
    clearing: "ПроцедураОчистки",
    tuning: "ПроцедураРегулирования",
    creating: "ПроцедураСоздания",
    multipleValuesDelete: "ПроцедураУдаленияМножественныхЗначений",
  },
}

export const fullInputFieldPartialEnterprise: InputFieldPartialEnterprise = {
  АвтоВыборНезаполненного: "Истина",
  АвтоИзменениеРегистраПриВводеТекста: "ВсеСимволы",
  АвтоИсправлениеПриВводеТекста: "Использовать",
  АвтоМаксимальнаяВысота: "Истина",
  АвтоМаксимальнаяШирина: "Истина",
  АвтоОтметкаНезаполненного: "Истина",
  АвтоОтображениеКнопкиОткрытия: "ТолькоДляЗаполненного",
  АвтоОтображениеКнопкиОчистки: "Всегда",
  АвтоПереносСтрок: "Истина",
  БыстрыйВыбор: "Истина",
  ВариантУправленияВысотой: "ПоСодержимому",
  ВыбиратьТип: "Истина",
  ВыборГруппИЭлементов: "ГруппыИЭлементы",
  ВыделенныйТекст: "Выделенный текст",
  ВыделятьОтрицательные: "Истина",
  Высота: 100,
  ВысотаСпискаВыбора: 200,
  ГиперссылкаМножественныхЗначений: "Истина",
  ДоступныеТипы: "Документ.КакойТоДокумент",
  ОграничениеТипа: "Строка",
  ИсторияВыбораПриВводе: "НеИспользовать",
  КартинкаКнопкиВыбора: "Печать",
  КартинкаМножественныхЗначений: "ОбщаяКартинка",
  КнопкаВыбора: "Истина",
  КнопкаВыпадающегоСписка: "Истина",
  КнопкаОткрытия: "Истина",
  КнопкаОчистки: "Истина",
  КнопкаРегулирования: "Истина",
  КнопкаСоздания: "Истина",
  КнопкаСпискаВыбора: "Истина",
  МаксимальнаяВысота: 500,
  МаксимальнаяШирина: 400,
  МаксимальноеЗначение: 100,
  Маска: "999",
  МинимальноеЗначение: 0,
  МногострочныйРежим: "Истина",
  ОбновлениеТекстаРедактирования: "ПриИзмененииЗначения",
  ОтметкаНезаполненного: "Истина",
  ОтображатьФлажкиВВыпадающемСпискеПриВводеМножественныхЗначений: "Истина",
  ОтображениеКнопкиВыбора: "ОтображатьВПолеВвода",
  ПараметрыВыбора: {
    "Отбор.Параметр": '"Значение"',
  },
  ПодсказкаАвтозаполнения: "Email",
  ПодсказкаВвода: "Подсказка ввода",
  ПроверкаПравописанияПриВводеТекста: "Использовать",
  ПутьКДаннымЗначенияМножественногоЗначения: "Объект.Value",
  ПутьКДаннымКартинкиМножественногоЗначения: "Объект.Picture",
  ПутьКДаннымПредставленияМножественногоЗначения: "Объект.Presentation",
  РазмерКартинкиМножественногоЗначения: "Крупный",
  РазрешитьВводПустыхМножественныхЗначений: "Истина",
  РазрешитьДублированиеМножественныхЗначений: "Ложь",
  РазрешитьСоставнойТип: "Истина",
  РастягиватьПоВертикали: "Истина",
  РастягиватьПоГоризонтали: "Истина",
  РасширенноеРедактирование: "Истина",
  РасширенноеРедактированиеМножественныхЗначений: "Истина",
  РедактированиеТекста: "Истина",
  РежимВыбораИзСписка: "Истина",
  РежимВыбораНезаполненного: "ПриАктивизации",
  РежимПароля: "Истина",
  СвязиПараметровВыбора: "Отбор.Владелец(Справочник.Справочник.Реквизит.Реквизит)",
  СвязьПоТипу: "Справочник.КакойТоСправочник.Реквизит.КакойТоРеквизит(1)",
  СпециальныйРежимВводаТекста: "Email",
  СписокВыбора: ['"Значение1"(Значение 1)'],
  ТекстКнопкиВводаЭкраннойКлавиатуры: "Готово",
  ТекстРедактирования: "Текст редактирования",
  ФигураКартинкиМножественногоЗначения: "Круг",
  ФормаВыбора: "Catalog.Справочник.Form.ФормаВыбора",
  Формат: "Формат отображения",
  ФорматРедактирования: "Формат редактирования",
  ЦветРамки: "Зеленый",
  ЦветТекста: "Желтый",
  ЦветТекстаМножественныхЗначений: "Красный",
  ЦветФона: "Синий",
  ЦветФонаМножественныхЗначений: "Фуксин",
  Ширина: 200,
  ШиринаВыпадающегоСписка: 300,
  Шрифт: "ОбычныйШрифтТекста",
  ШрифтМножественныхЗначений: "ОбычныйШрифтТекста",
  АвтоВысотаЯчейки: "Истина",
  АктивизироватьПоУмолчанию: "Истина",
  ВажностьПриОтображении: "Высокая",
  ВертикальноеПоложение: "Верх",
  ВертикальноеПоложениеВГруппе: "Верх",
  Вид: "ПолеВвода",
  Видимость: "Истина",
  ВысотаЗаголовка: 20,
  ГиперссылкаЯчейки: "Истина",
  ГоризонтальноеПоложение: "Лево",
  ГоризонтальноеПоложениеВГруппе: "Лево",
  ГоризонтальноеПоложениеВПодвале: "Лево",
  ГоризонтальноеПоложениеВШапке: "Лево",
  Доступность: "Истина",
  КартинкаПодвала: "Печать",
  КартинкаШапки: "Печать",
  ОтображатьВПодвале: "Истина",
  ОтображатьВШапке: "Истина",
  ОтображениеПодсказки: "Нет",
  ОтображениеПредупрежденияПриРедактировании: "НеОтображать",
  Подсказка: "Подсказка",
  ПоложениеЗаголовка: "Лево",
  РазрешитьИспользование: { Администратор: "Истина" },
  ПредупреждениеПриРедактировании: "Предупреждение",
  ПропускатьПриВводе: "Ложь",
  ПутьКДанным: "Объект.Реквизит",
  ПутьКДаннымПодвала: "Объект.РеквизитПодвала",
  РежимРедактирования: "ВходПриВводе",
  СочетаниеКлавиш: "Ctrl+S",
  Таблица: "Таблица",
  ТекстПодвала: "Текст подвала",
  ТолькоПросмотр: "Ложь",
  ФиксацияВТаблице: "Нет",
  ЦветТекстаЗаголовка: "Черный",
  ЦветТекстаПодвала: "Черный",
  ЦветФонаЗаголовка: "Синий",
  ЦветФонаПодвала: "Белый",
  ШрифтЗаголовка: "ОбычныйШрифтТекста",
  ШрифтПодвала: "ОбычныйШрифтТекста",
  КонтекстноеМеню: { Автозаполнение: "Ложь" },
  РасширеннаяПодсказка: { Заголовок: "Расширенная подсказка" },
  События: {
    ПриИзменении: "ПроцедураПриИзменении",
    АвтоПодбор: "ПроцедураАвтоПодбора",
    ДобавлениеМножественныхЗначений: "ПроцедураДобавленияМножественныхЗначений",
    ИзменениеТекстаРедактирования: "ПроцедураИзмененияТекстаРедактирования",
    НачалоВыбора: "ПроцедураНачалаВыбора",
    НачалоВыбораИзСписка: "ПроцедураНачалаВыбораИзСписка",
    ОбработкаВыбора: "ПроцедураОбработкиВыбора",
    ОбработкаНавигационнойСсылкиМножественногоЗначения: "ПроцедураОбработкиНавигационнойСсылкиМножественногоЗначения",
    ОбработкаФормированияКоманд: "ПроцедураОбработкиФормированияКоманд",
    ОкончаниеВводаТекста: "ПроцедураОкончанияВводаТекста",
    Открытие: "ПроцедураОткрытия",
    ОткрытиеМножественногоЗначения: "ПроцедураОткрытияМножественногоЗначения",
    Очистка: "ПроцедураОчистки",
    Регулирование: "ПроцедураРегулирования",
    Создание: "ПроцедураСоздания",
    УдалениеМножественныхЗначений: "ПроцедураУдаленияМножественныхЗначений",
  },
}

export const fullInputFieldTypedEnterprise: InputFieldTypedEnterprise = {
  ...fullInputFieldPartialEnterprise,
  Тип: "ПолеВвода",
  Заголовок: "Поле ввода",
}

export const minimalInputField: InputField = {
  elementType: FormElementType.InputField,
  name: "ПолеВвода",
}

export const minimalInputFieldPartialEnterprise: InputFieldPartialEnterprise = {}

export const minimalInputFieldTypedEnterprise: InputFieldTypedEnterprise = {
  Тип: "ПолеВвода",
}

export interface InputFieldStructureFixture {
  name: string
  element: InputField
  structured: IFormatElementResult
}

export const inputFieldStructureFixturesTable: InputFieldStructureFixture[] = [
  {
    name: "with title",
    element: {
      name: "ИмяПоля",
      elementType: FormElementType.InputField,
      title: { items: { ru: "Поле" } },
    },
    structured: {
      strings: ["Поле: {ИмяПоля}"],
      haveSimpleHorizontalGroup: false,
    },
  },
  {
    name: "without title",
    element: {
      name: "ИмяПоля",
      elementType: FormElementType.InputField,
    },
    structured: {
      strings: ["{ИмяПоля}: "],
      haveSimpleHorizontalGroup: false,
    },
  },
]

export const fullInputFieldPreview: Required<InputFieldPreview> = {
  ElementType: "InputField",
  Name: "ПолеВвода",
  AllowInputEmptyMultipleValues: true,
  AllowMultipleValuesDuplicates: false,
  AutoCapitalizationOnTextInput: { type: "AutoCapitalizationOnTextInput", value: "AllCharacters" },
  AutoChoiceIncomplete: true,
  AutoCorrectionOnTextInput: { type: "AutoCorrectionOnTextInput", value: "Use" },
  AutoFillHint: { type: "InputFieldAutofillHint", value: "Email" },
  AutoMarkIncomplete: true,
  AutoMaxHeight: true,
  AutoMaxWidth: true,
  AutoShowClearButton: { type: "AutoShowClearButtonMode", value: "Always" },
  AutoShowOpenButton: { type: "AutoShowOpenButtonMode", value: "FilledOnly" },
  BackColor: { type: "WebColor", value: "Blue" },
  BorderColor: { type: "WebColor", value: "Green" },
  ChoiceButton: true,
  ChoiceButtonPicture: { type: "StandardPicture", value: "Print" },
  ChoiceButtonRepresentation: { type: "ChoiceButtonRepresentation", value: "ShowInInputField" },
  ChoiceFoldersAndItems: { type: "FoldersAndItems", value: "FoldersAndItems" },
  ChoiceForm: "Catalog.Справочник.Form.ФормаВыбора",
  ChoiceHistoryOnInput: { type: "ChoiceHistoryOnInput", value: "DontUse" },
  ChoiceListButton: true,
  ChoiceListHeight: 200,
  ChooseType: true,
  ClearButton: true,
  CreateButton: true,
  DropListButton: true,
  DropListWidth: 300,
  EditFormat: "Формат редактирования",
  EditText: "Текст редактирования",
  EditTextUpdate: { type: "EditTextUpdate", value: "OnValueChange" },
  ExtendedEdit: true,
  Font: { Type: "Font", Name: "NormalTextFont" },
  Format: "Формат отображения",
  Height: 100,
  HeightControlVariant: { type: "HeightControlVariant", value: "UseContentHeight" },
  HorizontalStretch: true,
  IncompleteChoiceMode: { type: "IncompleteChoiceMode", value: "OnActivate" },
  InputHint: "Подсказка ввода",
  ListChoiceMode: true,
  MarkIncomplete: true,
  MarkNegatives: true,
  Mask: "999",
  MaxHeight: 500,
  MaxValue: 100,
  MaxWidth: 400,
  MinValue: 0,
  MultiLine: true,
  MultipleValuePictureDataPath: "Объект.Picture",
  MultipleValuePictureShape: { type: "MultipleValuePictureShape", value: "Circle" },
  MultipleValuePictureSize: { type: "MultipleValuePictureSize", value: "Large" },
  MultipleValuePresentationDataPath: "Объект.Presentation",
  MultipleValuesBackColor: { type: "WebColor", value: "Magenta" },
  MultipleValuesExtendedEdit: true,
  MultipleValuesFont: { Type: "Font", Name: "NormalTextFont" },
  MultipleValuesHyperlink: true,
  MultipleValuesPicture: { type: "CommonPicture", value: "ОбщаяКартинка" },
  MultipleValuesTextColor: { type: "WebColor", value: "Red" },
  MultipleValueValueDataPath: "Объект.Value",
  OnScreenKeyboardReturnKeyText: { type: "OnScreenKeyboardReturnKeyText", value: "Done" },
  OpenButton: true,
  PasswordMode: true,
  QuickChoice: true,
  SelectedText: "Выделенный текст",
  ShowCheckBoxesInDropListWhenInputMultipleValues: true,
  SpecialTextInputMode: { type: "SpecialTextInputMode", value: "Email" },
  SpellCheckingOnTextInput: { type: "SpellCheckingOnTextInput", value: "Use" },
  SpinButton: true,
  TextColor: { type: "WebColor", value: "Yellow" },
  TextEdit: true,
  TypeDomainEnabled: true,
  VerticalStretch: true,
  Width: 200,
  Wrap: true,
  AutoCellHeight: true,
  CellHyperlink: true,
  DataPath: "Объект.Реквизит",
  DefaultItem: true,
  DisplayImportance: { type: "DisplayImportance", value: "High" },
  EditMode: { type: "EditMode", value: "EnterOnInput" },
  Enabled: true,
  FixingInTable: { type: "FixingInTable", value: "None" },
  FooterBackColor: { type: "WebColor", value: "White" },
  FooterDataPath: "Объект.РеквизитПодвала",
  FooterFont: { Type: "Font", Name: "NormalTextFont" },
  FooterHorizontalAlign: { type: "HorizontalAlign", value: "Left" },
  FooterPicture: { type: "StandardPicture", value: "Print" },
  FooterText: "Текст подвала",
  FooterTextColor: { type: "WebColor", value: "Black" },
  HeaderHorizontalAlign: { type: "HorizontalAlign", value: "Left" },
  HeaderPicture: { type: "StandardPicture", value: "Print" },
  HorizontalAlign: { type: "HorizontalAlign", value: "Left" },
  HorizontalAlignInGroup: { type: "HorizontalAlign", value: "Left" },
  ReadOnly: false,
  ShowInFooter: true,
  ShowInHeader: true,
  SkipOnInput: false,
  Title: "Поле ввода",
  TitleBackColor: { type: "WebColor", value: "Blue" },
  TitleFont: { Type: "Font", Name: "NormalTextFont" },
  TitleHeight: 20,
  TitleLocation: { type: "TitleLocation", value: "Left" },
  TitleTextColor: { type: "WebColor", value: "Black" },
  ToolTip: "Подсказка",
  ToolTipRepresentation: { type: "ToolTipRepresentation", value: "None" },
  VerticalAlign: { type: "VerticalAlign", value: "Top" },
  VerticalAlignInGroup: { type: "VerticalAlign", value: "Top" },
  Visible: true,
  WarningOnEdit: "Предупреждение",
  WarningOnEditRepresentation: { type: "WarningOnEditRepresentation", value: "DontShow" },
}
