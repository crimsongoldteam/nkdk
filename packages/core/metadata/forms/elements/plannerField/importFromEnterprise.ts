...
export function importPlannerFieldTypedFromEnterprise<To extends PlannerField | undefined>(
  ...
  const props = importPlannerFieldPropsFromEnterprise(context, undefined, data)
...
const importPlannerFieldPropsFromEnterprise = (
  ...
  const dimensionItemHyperlink = importBooleanFromEnterprise(context, undefined, data.ГиперссылкаЭлементаИзмерения)
  if (dimensionItemHyperlink !== undefined) result.dimensionItemHyperlink = dimensionItemHyperlink

  const timeScaleItemHyperlink = importBooleanFromEnterprise(context, undefined, data.ГиперссылкаЭлементаШкалыВремени)
