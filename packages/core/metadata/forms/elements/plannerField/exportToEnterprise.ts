...
type PlannerFieldCommonFields = Omit<PlannerField, "elementType" | "name">

export const exportPlannerFieldPartialToEnterprise = <From extends PlannerField | undefined>(
  ...
  const dimensionItemHyperlink = exportBooleanToEnterprise(context, undefined, data.dimensionItemHyperlink)
  ...
  const timeScaleItemHyperlink = exportBooleanToEnterprise(context, undefined, data.timeScaleItemHyperlink)
  if (timeScaleItemHyperlink !== undefined) result.ГиперссылкаЭлементаШкалыВремени = timeScaleItemHyperlink

  const wrappedTimeScaleHeaderHyperlink = exportBooleanToEnterprise(
    context,
    undefined,
...
const exportPlannerFieldCommonFieldsToEnterprise = (
  context: ConfigurationContext,
  _rule: PropertyRule<any>,
  data: PlannerFieldCommonFields
): PlannerFieldPartialEnterprise => {
  const result: PlannerFieldPartialEnterprise = {}
...
registerMetadata(
  ...
  "PlannerField",
  exportPlannerFieldPartialToEnterprise as ExportPartialToEnterpriseFn
)
