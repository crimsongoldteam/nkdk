...
describe("exportAutoCommandBarToXML", () => {
  ...
  describe("exportTableAutoCommandBarToXML", () => {
    it("should return default when data is undefined", () => {
      ...
      const xmlData = exportTableAutoCommandBarToXML(mockContext, undefined, parentElement)
    ...
    it("should return all fields to XML", () => {
      ...
      const xmlData = exportTableAutoCommandBarToXML(mockContext, fullAutoCommandBar, parentElement)
      const result = xmlExport({ AutoCommandBar: xmlData }, false)
      expect(result).toEqual(expectedResult)
    })
    ...
    it("should export minimal", () => {
      const expectedResult = readXMLFileAsString("forms/autoCommandBar/minimalTable.xml")
      const xmlData = exportTableAutoCommandBarToXML(mockContext, minimalAutoCommandBar, parentElement)
      const result = xmlExport({ AutoCommandBar: xmlData }, false)
      expect(result).toEqual(expectedResult)
    })
  })
