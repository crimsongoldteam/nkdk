...
export function importCheckBoxFieldTypedFromEnterprise<To extends CheckBoxField | undefined>(
  ...
  const props = importCheckBoxFieldPropsFromEnterprise(context, undefined, data)
...
const importCheckBoxFieldPropsFromEnterprise = (
  ...
  const checkBoxType = importSystemEnumerationFromYAML<SE.CheckBoxType>(
    context,
    undefined,
    data.ВидФлажка,
  ...
  const equalItemsWidth = importBooleanFromEnterprise(context, undefined, data.ОдинаковаяШиринаЭлементов)
  if (equalItemsWidth !== undefined) result.equalItemsWidth = equalItemsWidth

  const threeState = importBooleanFromEnterprise(context, undefined, data.ТриСостояния)
...
registerMetadata(
  ...
  "CheckBoxField",
  importCheckBoxFieldPartialFromEnterprise as ImportPartialFromEnterpriseFn
)
