...
export function importProgressBarFieldTypedFromEnterprise<To extends ProgressBarField | undefined>(
  ...
  const props = importProgressBarFieldPropsFromEnterprise(context, undefined, data)
...
const importProgressBarFieldPropsFromEnterprise = (
  ...
  const showPercent = importBooleanFromEnterprise(context, undefined, data.ОтображатьПроценты)
  if (showPercent !== undefined) result.showPercent = showPercent

  const representation = importSystemEnumerationFromYAML<SE.ProgressBarSmoothingMode>(
    context,
    data.Отображение,
...
registerMetadata(
  ...
  "ProgressBarField",
  importProgressBarFieldPartialFromEnterprise as ImportPartialFromEnterpriseFn
)
