...
const rule: PropertyRule = { type: "MetadataWSReference", yaml: "WSСсылка" }

describe("import MetadataWSReference from YAML", () => {
  ...
  it("imports full fixture", () => {
    ...
    expect(result).toEqual({ ...full, name: undefined, internalInfo: undefined })
  })
  ...
  it("round-trip: full — import затем export даёт тот же YAML (parsed)", () => {
    ...
    expect(exported).toEqual({ WSСсылка: fullYAML })
  })
})
