...
describe("migration paths", () => {
  ...
  it("rejects rename target matching current local name", () => {
    expect(() => buildRenameTargetPath("Справочник.Товары", "Товары")).toThrow("Переименование в то же имя запрещено")
  })
  ...
  it("rejects child paths for simple top level applied objects", () => {
    ...
    expect(() => parseMigrationPath("ОбщийРеквизит.Организация.Реквизит.Код")).toThrow("Неподдерживаемый путь миграции")
    expect(() => parseMigrationPath("Бот.Телеграм.Реквизит.Код")).toThrow("Неподдерживаемый путь миграции")
    expect(() => parseMigrationPath("WSСсылка.Калькулятор.Форма.Основная")).toThrow("Неподдерживаемый путь миграции")
  })
