...
const rule: PropertyRule = { type: "MetadataFilterCriterion", yaml: "КритерийОтбора" }

describe("import MetadataFilterCriterion from YAML", () => {
  ...
  it("round-trip: full — import затем export даёт тот же YAML (parsed)", () => {
    ...
    expect(exported).toEqual({ КритерийОтбора: fullYAML })
  })
})
