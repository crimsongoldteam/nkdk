...
const rule: PropertyRule = { type: "MetadataEventSubscription", yaml: "ПодпискаНаСобытие" }

describe("export MetadataEventSubscription to YAML", () => {
  ...
  it("exports full fixture", () => {
    ...
    expect(result).toEqual({ ПодпискаНаСобытие: fullYAML })
  })

  it("exports minimal fixture", () => {
    const result = testExportPropertyToYAML({ rule, value: minimal })
    expect(result).toEqual({ ПодпискаНаСобытие: minimalYAML })
  })
})
