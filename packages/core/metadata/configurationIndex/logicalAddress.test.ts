...
describe("configuration index logical address", () => {
  it("builds semantic addresses used by reference-order-spec", () => {
    ...
    expect(childSegmentUid("Документ.ПоступлениеТоваровУслуг.Форма.ФормаДокумента.Элемент.Товары", "КонтекстноеМеню")).toBe(
      "Документ.ПоступлениеТоваровУслуг.Форма.ФормаДокумента.Элемент.Товары.КонтекстноеМеню"
    )
