...
function target(root: string): MetadataTarget {
  const itemDir = join(root, "packages/rules/metadata/appliedObjects/metadataDocument")
...
describe("fixtureCopier", () => {
  ...
  it("formatTestCommands возвращает команды для проверки XML-конвейера", () => {
    expect(formatTestCommands("metadataDocument")).toEqual([
      "pnpm --filter @nkdk/rules exec vitest run packages/rules/metadata/appliedObjects/metadataDocument/fromXML.test.ts",
      "pnpm --filter @nkdk/rules exec vitest run packages/rules/metadata/appliedObjects/metadataDocument/toXML.test.ts",
      "pnpm --filter @nkdk/rules exec vitest run packages/rules/metadata/appliedObjects/metadataDocument/convertFromXML.test.ts",
      "pnpm --filter @nkdk/rules exec vitest run packages/rules/metadata/appliedObjects/metadataDocument/syncToXML.test.ts",
    ])
  })
