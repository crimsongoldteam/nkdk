...
export function formatTestCommands(metadataItem: string): string[] {
  const testDir = `packages/rules/metadata/appliedObjects/${metadataItem}`

  return [
    `pnpm --filter @nkdk/rules exec vitest run ${testDir}/fromXML.test.ts`,
    `pnpm --filter @nkdk/rules exec vitest run ${testDir}/toXML.test.ts`,
    `pnpm --filter @nkdk/rules exec vitest run ${testDir}/convertFromXML.test.ts`,
    `pnpm --filter @nkdk/rules exec vitest run ${testDir}/syncToXML.test.ts`,
  ]
}
