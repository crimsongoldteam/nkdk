...
type ConversionResult = ReturnType<typeof testPropertyFixtureThroughYAML>
const conversions = new Map<string, ConversionResult>()

beforeAll(() => {
  for (const fixture of ["full.xml", "minimal.xml"]) {
    conversions.set(fixture, convertFixture(fixture))
  }
})

describe("MetadataExternalDataSourceCube XML → YAML → XML", () => {
  it.each(["full.xml", "minimal.xml"])("round-trips %s", (fixture) => {
    const result = requireConversion(fixture)
  ...
  it("imports dimension table child names into dimensionTables without dimensionTableNames", () => {
    const result = requireConversion("full.xml")
...
function convertFixture(fixture: string): ConversionResult {
...
function requireConversion(fixture: string): ConversionResult {
  const result = conversions.get(fixture)
  if (result === undefined) throw new Error(`Не подготовлена тестовая конвертация ${fixture}`)
  return result
}
