...
import { exportOperationItemToYamlText, writeOperationYamlFile } from "./yamlIO"

describe("metadata operation YAML IO", () => {
  ...
  it("exports and writes a touched metadata item directly", async () => {
    ...
    const text = exportOperationItemToYamlText(item)
    expect(text).toContain("Реквизиты:")
    expect(text).toContain("Артикул:")

    writeOperationYamlFile(item)
