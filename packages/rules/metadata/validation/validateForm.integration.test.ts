...
describe("validateForm", () => {
  ...
  it("запрещает избыточный вычисляемый ПутьКДанным через системное перечисление", () => {
      const project = createProject({
        owner: ["Комментарий: Владелец формы"],
        form: [
          "Реквизиты:",
          "  Объект:",
      ...
      expect(runValidateForm(project)).toEqual(expect.arrayContaining([
        expect.objectContaining({
          path: "/Элементы/Наименование/ПутьКДанным",
          severity: "error",
          source: "structure",
  ...
  it("разрешает пустой и отличный путь собственного элемента", () => {
    const project = createProject({
      owner: ["Комментарий: Владелец формы"],
      form: [
        "Реквизиты:",
    ...
    expect(messages(runValidateForm(project))).not.toContainEqual(expect.stringContaining("не нужно указывать явно"))
  })

  it("принимает конечные пути к произвольному реквизиту и колонке", () => {
    const project = createProject({
      form: [
        "Реквизиты:",
  ...
  it("запрещает продолжение пути после произвольной колонки", () => {
    const project = createProject({
      form: [
        "Реквизиты:",
        "  Таблица:",
        ...
        "  Поле:",
        "    Вид: ПолеВвода",
        "    ПутьКДанным: Таблица.Значение.Поле",
      ],
    })

    expect(messages(runValidateForm(project))).toContain(
      'ПутьКДанным "Таблица.Значение.Поле": промежуточный реквизит "Значение" имеет произвольный тип'
    )
  })
  ...
  it("accepts StandardPeriod platform fields", () => {
    const project = createProject({
      form: [
        ...
        "  Вариант:",
        "    Вид: ПолеВвода",
        "    ПутьКДанным: Период.Вариант",
        "  ДатаНачала:",
        "    Вид: ПолеВвода",
        "    ПутьКДанным: Период.ДатаНачала",
        "  ДатаОкончания:",
        "    Вид: ПолеВвода",
        "    ПутьКДанным: Период.ДатаОкончания",
      ],
    })
  ...
  it("использует вычисляемый путь таблицы для Элементы.*.ТекущиеДанные", () => {
    const project = createProject({
      ownerDir: "Документ",
      ownerName: "Заказ",
      owner: ["ТабличныеЧасти:", "  Товары:", "    Реквизиты:", "      Сумма:", "        Тип: Число"],
      form: [
        ...
        "  Кнопка:",
        "    Вид: Кнопка",
        "    Данные: Элементы.Товары.ТекущиеДанные.Сумма",
      ],
    })
