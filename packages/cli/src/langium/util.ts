...
/**
 * Парсит NKDK-строку в AST формы (Form).
 * При пустой строке или ошибке парсинга возвращает undefined.
 */
export async function parseNKDKFromString(content: string): Promise<Form | undefined> {
    const trimmed = content?.trim() ?? '';
    if (trimmed.length === 0) {
        return undefined;
    }
    const services = createNkdkServices(EmptyFileSystem).Nkdk;
    const parse = parseHelper<Form>(services);
    const document = await parse(trimmed);
    const value = document.parseResult?.value;
    if (document.parseResult.parserErrors.length > 0 || value === undefined) {
        return undefined;
    }
