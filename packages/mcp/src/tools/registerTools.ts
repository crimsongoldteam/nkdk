...
export function registerNkdkCapabilities(server: RegisterableServer): void {
  ...
  server.registerTool(
    "nkdk.import_from_xml",
    {
      ...
      description:
        "Импортирует готовую XML-выгрузку одного компонента из xmlDir в projectDir/componentPath. componentPath по умолчанию cf; цель должна отсутствовать или быть пустой. Операция не подключается к 1С и не импортирует все компоненты за один вызов. При неуспехе сохраняет временные файлы в .nkdk/tmp/import/<operation-id>. Пишет файлы только при allowWrite=true.",
