...
export const guideDefinitions: GuideDefinition[] = [
  {
    ...
    text: [
      ...
      "",
      "Если пользователь хочет переименовать metadata-объект, реквизит, табличную часть, форму или макет, не правь YAML руками: вызови `nkdk.rename_item`. Если пользователь хочет проверить удаление, вызови `nkdk.delete_item`: сейчас инструмент ищет внешние ссылки и не удаляет файлы.",
