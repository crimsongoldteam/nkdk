...
2. Храни **все тестовые фикстуры** в `__fixtures__/data.ts` (включая модели, YAML-представления, `PropertyRule` и другие входные/ожидаемые данные). Не объявляй fixture-объекты внутри `*.test.ts`.
   - Не делай постобработку ожидаемого/фактического результата в `*.test.ts`: запрещены `expect({ ...fixture, ... })`, ручные дописывания полей, `map`/`omit`/`pick` и другие трансформации перед сравнением. Любые отличия должны быть выражены отдельной fixture в `__fixtures__/data.ts`.
   - Фикстуры для импорта и экспорта должны быть согласованной парой: одна и та же модель (`item`) и её соответствующее представление (`xml`/`yaml`) используются в тестах обоих направлений без ручной подгонки в тестах.
