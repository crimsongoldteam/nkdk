...
Порядок самих элементов `NavigationPanel` и `CommandBar` пока остаётся отдельным вопросом. В референсных XML встречаются группы, где физический порядок `<Item>` не совпадает с возрастанием `Index`, поэтому нельзя без дополнительного исследования сортировать элементы только по `Index`, `CommandGroup` или позиции внутри группы. Будущая доработка должна зафиксировать правило восстановления физического порядка по нескольким каталогам `/Users/nikita/git/round-trip-source/` и только после этого менять сортировку `fromXML`/`toXML`.
