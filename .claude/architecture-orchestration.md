...
YAML остаётся целевым состоянием проекта и не хранит `_uuid`. Миграции фиксируют только те структурные решения, которые нельзя безопасно вывести из сравнения YAML и эталонного XML: было ли сочетание «удалено + добавлено» на одном уровне переименованием, удалением или добавлением.

Миграции хранятся в YAML-проекте в каталоге `Миграции/`. Один файл — одна миграция, идентификатор миграции — имя файла. Формат имени: `YYYY-MM-DD-HHmmss-<оттенок-жёлтого>-<забавное-существительное>.yaml`; случайный хвост нужен только для читаемого уникального имени, порядок применения задаёт дата-время.
...
Если в миграции есть переименование родителя, дочерние строки пишутся через новое имя родителя. Например, после `"Справочник.Товары": "Номенклатура"` реквизит внутри нового объекта записывается как `"Справочник.Номенклатура.Реквизит.Артикул": "НовыйАртикул"`. Применение миграций поэтому строит remap сверху вниз, а не трактует каждую строку изолированно.
...
В первой версии миграции покрывают:

- верхнеуровневые объекты из `TopLevelMetadataItemRules`: `Справочник`, `Документ`, `Нумератор`, `Последовательность`;
- дочерние owning-коллекции с `uuid` и `keyField: "name"`: `Реквизит`, `ТабличнаяЧасть`, реквизиты табличной части, `Измерение`.
...
Если на уровне есть только удаления или только добавления, миграция не нужна: `sync` применяет целевое YAML-состояние напрямую. Если на уровне одновременно есть удаления и добавления, это конфликт намерения: `sync` обязан остановиться, пока миграция явно не разметит элементы как переименование, удаление или добавление.

`generate-migration <yaml-dir> <xml-dir>` сравнивает XML как прошлое состояние и YAML как целевое состояние. Команда идёт сверху вниз, интерактивно разбирает только конфликтные уровни и пишет файл миграции только после полного завершения сессии. Если пользователь прерывает сессию, файл не создаётся. Режим `--dry-run` только показывает найденные конфликты и ничего не пишет.

В интерактивном режиме генератор для каждого удалённого элемента предлагает выбрать одну из добавленных целей как переименование или выбрать `Удалить`. Выбранная цель убирается из списка добавленных. Все добавленные элементы, оставшиеся после разбора удалённых, записываются как `Добавить`. Автоматического угадывания переименований нет, даже для случая «одно удаление + одно добавление».
...
`rename` пишет строку `"path": "new-name"`. `delete` пишет строку `"path": Удалить`. Дубликаты не проверяются: повторная миграция при `sync` должна стать предупреждением и продолжить выполнение.

Каждый XML-каталог хранит своё состояние применённых миграций в корневом файле `.nakidka-migrations.yaml`:

```yaml
applied:
  - "2026-05-05-143000-янтарный-бублик.yaml"
```

Хранятся только имена файлов, без хэша содержимого. Если применённая миграция исчезла из YAML-проекта, `sync` молчит. Если в YAML-проекте есть миграции, которых нет в `.nakidka-migrations.yaml`, `sync` применяет их автоматически по имени файла в порядке сортировки и после успешного применения добавляет имя файла в список `applied`.

Миграции не должны мутировать XML напрямую до экспорта. Они строят remap для поиска эталонных XML-данных:

- при переименовании верхнеуровневого объекта новый YAML-объект читает `referenceData` из старого XML-файла;
- при переименовании дочернего элемента коллекции `exportMetadataCollectionToXML` должен искать `referenceItem` по старому имени из миграции, а не только по текущему `keyField`;
- при удалении и добавлении миграция подтверждает намерение на конфликтном уровне.

Если `rename`-миграция не может найти новый элемент в текущем YAML, это ошибка: пользователь зафиксировал переименование, но не привёл YAML к целевому состоянию. Если строка миграции уже неприменима для конкретного XML-каталога, это предупреждение и продолжение; файл миграции всё равно считается применённым.

После экспорта `sync` выполняет обязательную очистку лишних верхнеуровневых XML-файлов и папок объектов, которых больше нет в YAML с учётом миграций. Реквизиты, табличные части и измерения очищаются естественно через перезапись XML-файла владельца.
