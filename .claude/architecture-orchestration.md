...
Миграции хранятся в YAML-проекте в каталоге `Миграции/`. Один файл — одна миграция, идентификатор миграции — имя файла. Формат имени: `YYYY-MM-DD-HHmmss.yaml` в UTC, без текстового хвоста. При создании нового файла берётся `max(nowUtc, latestMigrationTimestamp + 1s)`. Файлы с неправильным именем в `Миграции/` молча игнорируются.
...
Строки внутри файла применяются в порядке YAML-словаря. Дубликаты ключей отдельно не ловятся: YAML-парсер оставляет последнее значение. Каждая строка миграции должна быть применима к текущему промежуточному состоянию; любые несоответствия считаются ошибкой. Предупреждений применения нет.
...
`rename` пишет строку `"path": "new-name"`. `delete` пишет строку `"path": Удалить`. Ручные `rename`/`delete` не валидируют существование пути в XML/YAML. `rename` проверяет непустое новое имя, отсутствие точки и запрет no-op.
...
Хранятся только имена файлов, без хэша содержимого. В `.nakidka-migrations.yaml` допустимы только корректные имена миграций; дубликаты в `applied` — ошибка. Если применённая миграция исчезла из YAML-проекта, `sync` молчит и не читает такой файл. Если в YAML-проекте есть миграции, которых нет в `.nakidka-migrations.yaml`, `sync` применяет их автоматически по имени файла в порядке сортировки. `.nakidka-migrations.yaml` обновляется только после успешного экспорта и очистки.
...
После экспорта `sync` выполняет обязательную очистку лишних XML-файлов и папок по manifest ожидаемых файлов, только внутри известных `xmlDir`. Реквизиты, табличные части и измерения очищаются естественно через перезапись XML-файла владельца.
