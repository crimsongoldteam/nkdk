...
После EXECUTION и перед COMMIT обязательно прогнать `pnpm type-check` из корня репозитория. Это `pnpm -r exec tsc --noEmit` по всем пакетам.

Если `type-check` падает:
- чинить **только** те ошибки типов, которые напрямую связаны с изменениями текущей задачи;
- «чужие» ошибки (в коде, которого задача не касалась) не трогать — зафиксировать их отдельным комментарием к issue и продолжать;
- после правок повторно прогнать `pnpm type-check` до зелёного;
- только после зелёного `type-check` переходить к COMMIT.

Если за разумное число попыток свести `type-check` к зелёному не удаётся — COMMIT не делать, оставить комментарий в issue (секция THE ISSUE) с описанием оставшихся ошибок и выйти.

# COMMIT

Сформируй git-коммит по правилам скилла `.agents/skills/commit/SKILL.md` (тип + эмодзи-shortcode + описание в инфинитиве на русском), но с префиксом `RALPH:` в самом начале заголовка.
