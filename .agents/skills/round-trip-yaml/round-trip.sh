...
mcp_project_dir_for() {
  local rel
  rel="$(config_rel_path "${NKDK_XML_DIR}")"
...
link_mcp_component() {
  ...
  local component_dir="${project_dir}/${component_path}"
...
for RUN_XML_DIR in "${RUN_DIRS[@]}"; do
  ...
  link_mcp_component "${MCP_PROJECT_DIR}" "${RUN_YAML_DIR}" "${RUN_COMPONENT_PATH}"
