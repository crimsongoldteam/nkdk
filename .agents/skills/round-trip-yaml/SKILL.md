...
description: Диагностирует полный metadata round-trip через YAML и показывает single/triage diff'ы без создания reproducer.
---
...
- **Чистый активный XML-каталог.** Перед прогоном проверяется только каталог, для которого выполняется round-trip. Изменения в `nkdk-core` и других каталогах XML-репозитория не блокируют запуск.
...
2. проверяет, что `NKDK_XML_REPO` является git-репозиторием;
3. рекурсивно определяет основную конфигурацию и расширения по `Configuration.xml`, сохраняя их пути компонентов (`cf`, `cfe/<имя>`);
4. проверяет чистоту каждого активного XML-каталога, не учитывая другие каталоги репозитория;
5. перед каждым прогоном восстанавливает активный XML-каталог из `HEAD`;
...
8. вызывает `nkdk.import_from_xml`;
9. вызывает `nkdk.sync_to_xml`;
...
12. собирает tracked-изменения XML через `git diff` и новые неигнорируемые файлы через `git ls-files --others --exclude-standard`; новые файлы входят в `DIFF_COUNT` и показываются в single- и triage-режимах;
...
Если активный XML-каталог содержит изменения — попроси пользователя сохранить или откатить их. Не запускай `git stash`, `git restore` или `git clean` сам.
...
При запуске нескольких конфигураций через `--all-configs` структура компонентов сохраняется внутри `NKDK_ROUND_TRIP_YAML_DIR`: основная конфигурация попадает в `cf`, расширение — в `cfe/<имя>`. Этот же путь передаётся MCP-командам импорта и синхронизации. Все компоненты последовательно обрабатываются в одном временном MCP-проекте, поэтому расширение видит ранее импортированную основную конфигурацию.
...
Исключение: reference-only файлы, которые намеренно не входят в YAML-договор, копируются из активного XML-каталога во временный XML-каталог после `sync` и до замены активного каталога. Сейчас это `.nakidka-migrations.yaml`, `Ext/ParentConfigurations.bin` и `Ext/ParentConfigurations/*.cf`.
...
- `nkdk.import_from_xml` падает;
- `nkdk.sync_to_xml` падает;
