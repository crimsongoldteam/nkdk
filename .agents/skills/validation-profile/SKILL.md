...
Если полная холодная перестройка и следующий вызов не помещаются в память одного процесса, не обозначай отдельный запуск на готовом снимке как warm. Сначала зафиксируй сбой двух последовательных вызовов, затем отдельным запуском измерь стоимость загрузки готового снимка; настоящий warm подтверждается только вторым вызовом того же процесса с `workerReuseMs`.

Профиль отдельно показывает `workerPoolCreateMs`, `workerReadyMs` и
`workerReuseMs`, чтобы холодный запуск worker не смешивался с обработкой файлов.
