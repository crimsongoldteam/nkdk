...
flowchart TD
  ...
  subgraph partialSyncJob["Воркер"]
    direction LR
    readBlocks["Прочитать нужные блоки<br/>снимка"] --> readYaml["Прочитать YAML"] --> toXml[["Преобразовать YAML → XML"]] --> document["Вернуть XML-документы"] --> fragment["Вернуть новый блок"]
  end
