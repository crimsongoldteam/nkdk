...
- Concrete-слои направлены так: `systemEnumerations` не достигает `forms` и `appliedObjects`, `commonObjects` не достигает `forms` и `appliedObjects`, `forms` не достигает `appliedObjects`.
- `resourceTopology/core` содержит только компиляцию и нейтральные проекции; `resourceTopology/adapters` связывает их с регистрациями property-правил и project specs.
- Runtime-сборка принадлежит `metadata/composition`. В неё входят регистрация core metadata, сборка project state и регистрация worker operations; обычные metadata-модули не импортируют composition roots.
...
| Определение проекта | Нижний metadata-слой `projectDefinition`: нормализация путей, project specs, ресурсы, схемы, локальные факты и подготовленные YAML-дескрипторы. Координационные сценарии остаются в `metadata/project` и зависят от определения проекта, но не наоборот. |
| Rule runtime | Нейтральный слой `ruleRuntime`, который исполняет декларативные property/item-правила и хранит их общие договоры и регистрации, не зная concrete metadata-реализаций. |
| Composition root | Модуль `metadata/composition`, который связывает concrete-регистрации, project state и worker operations в готовые runtime-зависимости. |
...
Локальный тег `!xml` передаёт явно зарегистрированное действие над формой XML. Фиксированная регистрация `itemType + propertyKey + value` сохраняет присутствие XML-значения, совпадающего с неявным YAML-default, либо отсутствие XML-тега, для которого правило иначе создаёт XML-default. Во втором случае транспортным значением служит пустой scalar. Когда одно и то же фиксированное действие относится ко всем владельцам точного типа свойства, оно может быть зарегистрировано по `propertyType`; такая регистрация не распространяется на другие типы. Отдельно согласованная регистрация `transportScalar` для пары `itemType + propertyKey` передаёт payload штатному преобразователю типа; допустимость payload определяет предметный модуль. Parser возвращает обычный scalar и хранит транспортную пометку отдельно от предметной модели.

Metadata ruleRuntime принимает тег только для явно зарегистрированной фиксированной тройки, точного `propertyType` либо пары со скалярным транспортом. Регистрация `itemType + propertyKey` имеет приоритет над регистрацией типа. Внутренняя validation schema допускает транспортную форму зарегистрированного свойства, внешняя схема подсказок её не содержит. Незарегистрированное применение является ошибкой импорта. Регистрация исключительна: каждое новое применение `!xml` предварительно согласовывается с разработчиком и располагается рядом с правилами конкретного свойства.
...
| Действие | Описание | Воркер |
|---|---|:---:|
| Выполнение import зависимого компонента | Использует тот же import, раннюю запись и read-only второй проход. Имя и `componentPath` расширения определяются из корневого XML, а перед открытием import session выполняются этапы Б1–Б6 для существующего проекта. Состояние `cf` используется для проверки готовности базового компонента, уточнения diagnostics и построения ожидаемой основы заимствованной формы, но не дополняет metadata targets, владельцев и поля `cfe` при dependency validation. Общие metadata-слои не получают частных условий по `cfe`, `ObjectBelonging`, `ExtendedConfigurationObject` или `BaseForm`. | ✓ |
...
| Действие | Описание | Воркер |
|---|---|:---:|
| Формирование полного XML расширения | Использует те же этапы Б1–Б6 и тот же полный XML-export выбранного компонента. Профиль расширения дополнительно читает подтверждённое состояние `cf`, применяет строгую компонентную видимость общего индекса и восстанавливает зарегистрированные сведения заимствования. Для `BaseForm` используется сохранённый изолированный YAML-спутник, если он существует; иначе основа проецируется из текущей формы `cf` и рабочей формы расширения без создания спутника. Соседние `cfe` в индексном представлении расширения не видны. | ✓ |
...
| <a id="artifact-spisok-zadanii-bazovyh-form"></a>Список заданий базовых форм | Заимствованные формы выбранного расширения, связанные с соответствующими файлами формы основной конфигурации, рабочими формами расширения и необязательными YAML-спутниками исторической основы. |
| <a id="artifact-sohranennaya-osnova-formy"></a>Сохранённая основа формы | Необязательный `БазоваяФорма.yaml` рядом с рабочим представлением дочерней или общей формы. Содержит смысловое тело исторической основы без UUID, локальных числовых идентификаторов и XML-служебных деталей; не публикует metadata facts в общий индекс. |
| <a id="artifact-podgotovlennoe-bazovoe-predstavlenie-formy"></a>Подготовленное базовое представление формы | Основа, прочитанная из сохранённого YAML-спутника либо заново построенная из текущей формы `cf` в контексте рабочей формы расширения и готовая к встраиванию зарегистрированным правилом формы. |
...
| <a id="artifact-chastichnyi-snimok-konfiguratsii-rasshireniya"></a>Частичный снимок конфигурации расширения | Worker-фрагмент одного пути `cfe` с собственными содержательными `entity` расширения, включая идентификаторы и невосстановимое XML-состояние `Extended`. При наличии `БазоваяФорма.yaml` локальные идентификаторы её XML-проекции сохраняются с `sourceProjectPath` спутника в отдельном логическом пространстве основы; metadata facts основы в общий индекс не входят. Представленные в YAML состояния `Notify` не сохраняются. |
