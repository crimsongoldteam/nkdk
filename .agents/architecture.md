...
flowchart TD
  ...
  subgraph second["Второй проход воркера"]
    ...
    detail --> local2[["Проверить YAML и назначить<br/>локальные invalid/important;<br/>подтверждённые границы закрыть"]]
    local2 --> semantic["Подготовить смысловой вклад<br/>без записи YAML"]
  end

  secondErrors{"Есть ошибки?"}
  semanticIndex["Зафиксировать смысловой индекс;<br/>проверить связи только для<br/>неподтверждённых границ"]
