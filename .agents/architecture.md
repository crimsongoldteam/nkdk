...
|---|---|
| XML ↔ YAML без потерь | Поддержанный XML проходит `XML → YAML → XML` без потери содержимого. Необратимая XML-форма сохраняется общим `!xml/raw`; обратимая форма может храниться кратким тегом, явно зарегистрированным для точного типа свойства или элемента, например `!xml/standard-attributes`, `!xml/string` или `!xml/name`; обратимые смысловые ошибки сохраняются `!xml/invalid` или явно зарегистрированным `!xml/important`. UUID-ссылки на метаданные сохраняются отдельным `!xml/uuid` без разрешения через индекс. Договор перечислен в [реестре XML-аномалий](xml-anomalies.md). |
...
`RuleRegistrySet.execution` один раз компилирует для каждого
`MetadataItemRule` единый неизменяемый план преобразования свойств. Один план
содержит статические маршруты и разрешённые обработчики XML → YAML и
YAML → XML; динамические значения, контексты, индексы и XML/YAML-деревья в нём
не сохраняются. План принадлежит конкретному собранному runtime и worker и
пересобирается при изменении версии реестра обработчиков. Нейтральный runtime
не содержит частных условий по типам свойств: оптимизированные предметные
преобразователи регистрируются рядом с конкретными правилами.
...
flowchart TD
  ...
  subgraph first["Первый проход воркера"]
    ...
    itemVariant --> facts["По rules.ts собрать только полные факты<br/>семантического и dependency-индексов"]
  ...
  semanticDecisions["Зафиксировать индексы, проверить связи<br/>и классифицировать XML-решения"]
...
Факты первого прохода образуют те же семантический и dependency-индексы, которые
обычная проверка получает из полного YAML: цели, владельцы, поля, формы,
логические адреса и записи зависимостей. Первый проход ничего не проверяет и не
классифицирует. После фиксации общего снимка отдельная стадия проверяет связи,
поэтому межфайловые решения готовы до второго прохода. Второй проход дополняет их локальными решениями
текущего YAML и записывает файл один раз; повторное чтение записанного YAML для
классификации XML-аномалий не требуется.

Во втором проходе UUID-ссылка на метаданные получает точный `!xml/uuid` сразу
при построении YAML, до контрольного экспорта и без повторного чтения файла.
Первый проход такую ссылку не классифицирует и сохраняет прежний договор сбора
только индексов и фактов.
