...
Локальный тег `!xml` передаёт явно зарегистрированное действие над формой XML. Parser возвращает обычный scalar и хранит транспортную пометку отдельно от предметной модели.

Фиксированная регистрация `itemType + propertyKey + value` сохраняет присутствие XML-значения, совпадающего с неявным YAML-default, либо отсутствие XML-тега, для которого правило иначе создаёт XML-default. Во втором случае транспортным значением служит пустой scalar; допустимость действия проверяет ruleRuntime до преобразования XML.

Отдельно согласованная регистрация `transportScalar` задаётся точной парой `itemType + propertyKey`. RuleRuntime только снимает `!xml`, передаёт payload штатному fromYAML/toXML и восстанавливает тег при записи изменённого scalar; допустимость payload проверяет предметный metadata-модуль. Внутренняя validation schema принимает зарегистрированный транспортный scalar, а внешняя схема подсказок не предлагает ни `!xml`, ни служебные варианты значений.

Незарегистрированное применение является ошибкой импорта. Регистрация исключительна: каждое новое применение `!xml` предварительно согласовывается с разработчиком и располагается рядом с правилами конкретного свойства.
...
Явно зарегистрированное XML-only свойство может использовать пустой scalar с `!xml` как чисто транспортное действие без смыслового значения. Такое поле создаётся и потребляется только XML ↔ YAML импортом/экспортом, исключается из JSON Schema и полностью игнорируется semantic validation.
