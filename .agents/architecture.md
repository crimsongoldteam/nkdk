...
flowchart TD
  ...
  subgraph first["Первый проход воркера"]
    direction TD
    readXml["Прочитать XML совместимым парсером<br/>и потоково вычислить хэши корней"] --> itemVariant["До свойств определить вариант<br/>текущего metadata item"]
    itemVariant --> toYaml["По rules.ts построить<br/>смысловой YAML"]
