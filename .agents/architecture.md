...
Этапы Б2 и Б3 выполняются за один разбор каждого изменённого YAML: worker добавляет нормализованные сведения индекса, локальные diagnostics и отложенные проверки в единый двоичный накопитель своей фазы. Разделение на этапы фиксирует разные результаты этого прохода, а не дополнительное чтение файла. Следующая пачка одной worker-линии не начинается до завершения предыдущей; после окончания фазы линия передаёт накопленный фрагмент главному процессу без промежуточного восстановления предметных объектов.
...
Один MCP-процесс лениво создаёт один `ProjectStateService` и один универсальный пул worker метаданных; при переключении `projectDir` ранее активное состояние закрывается, а двоичный снимок нового проекта загружается в общие буферы и устанавливается в worker. Пул лениво увеличивается до наибольшей запрошенной параллельности и не уничтожает лишние прогретые линии при последующем уменьшении. Единственный признак совместимости состояния — версия формата `0.4.2`; отдельного отпечатка схем и правил нет.
...
| Первый проход import | Ограниченными пачками читает XML и для каждого задания формирует YAML, сведения основного индекса, локальные diagnostics и данные компонентного снимка конфигурации. Локальная validation использует тот же общий модуль и уже готовый объект YAML, что и validation изменённого файла. | ✓ |
| Ранняя запись готового YAML | Если YAML не содержит отложенных значений, worker сериализует и записывает его в первом проходе, вычисляет хэш по записываемым байтам, проверяет тот же объект без повторного чтения и разбора файла, добавляет результат в накопленный двоичный фрагмент и освобождает YAML. До второго прохода остаются только YAML с отложенными значениями. | ✓ |
...
| Второй проход только для чтения | Каждый worker использует установленные ссылки на неизменяемый рабочий индекс, уточняет принадлежащие ему отложенные значения, записывает и проверяет оставшиеся YAML, добавляет окончательные сведения в новый двоичный накопитель и освобождает объекты. | ✓ |
...
Главный процесс не получает дублирующие предметные `ownerFacts`, `validationContribution` или разобранные YAML и не удерживает список результатов всех import-заданий: те же сведения накапливаются в двоичном фрагменте worker. Профиль содержит агрегаты одной worker-линии и отдельные времена первого прохода, рабочего индекса, второго прохода, внешних файлов, окончательной сборки, dependency validation, публикации, сохранения и формирования результата MCP.
